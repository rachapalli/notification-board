...
public interface GroupRepo extends CrudRepository<Groups, Integer> {
	...
	@Query(value = "select u.email as user_email, u.user_name, g.group_name, ug.is_active, g.is_public from user_groups ug, users u, groups g where u.user_id = ug.user_id and ug.group_id = g.group_id and and ug.user_id != g.created_by and g.created_by=:userId")
